...
// Get pending investments (admin) - DEBUGGED VERSION
app.get('/api/admin/pending-investments', adminAuth, async (req, res) => {
  try {
    ...
    // DEBUGGED: Handle null users safely
    const safeInvestments = investments.map(inv => ({
      ...inv,
      user_details: inv.user ? {
        name: inv.user.full_name || 'Unknown User',
        email: inv.user.email || 'N/A',
        phone: inv.user.phone || 'N/A',
        current_balance: inv.user.balance || 0
      } : {
        name: 'User Deleted',
        email: 'N/A',
        phone: 'N/A',
      ...
      plan_name: inv.plan?.name || 'Plan Deleted',
      formatted_amount: `₦${inv.amount.toLocaleString()}`,
      has_proof: !!inv.payment_proof_url,
      proof_url: inv.payment_proof_url || null,
      days_pending: Math.ceil((new Date() - new Date(inv.createdAt)) / (1000 * 60 * 60 * 24))
    }));

    res.json(formatResponse(true, 'Pending investments retrieved', { investments: safeInvestments }));
  } catch (error) {
...
// Get pending deposits (admin) - DEBUGGED VERSION (THIS WAS THE PROBLEM AREA)
app.get('/api/admin/pending-deposits', adminAuth, async (req, res) => {
  try {
    ...
    console.log(`DEBUG: Found ${pendingDeposits.length} pending deposits`);
    
    // DEBUGGED: Add detailed logging to identify null users
    pendingDeposits.forEach((dep, index) => {
      if (!dep.user) {
        console.log(`DEBUG: Deposit ${index} has null user:`, {
          deposit_id: dep._id,
          amount: dep.amount,
    ...
    // DEBUGGED: Handle null users safely - FIX APPLIED HERE
    const enhancedDeposits = pendingDeposits.map(dep => ({
      ...dep,
      // SAFE ACCESS: Check if user exists before accessing properties
      user_details: dep.user ? {
        name: dep.user.full_name || 'Unknown User',
    ...
    console.log(`DEBUG: Processed ${enhancedDeposits.length} deposits successfully`);

    res.json(formatResponse(true, 'Pending deposits retrieved', { 
      deposits: enhancedDeposits,
      count: enhancedDeposits.length
    }));
  } catch (error) {
...
// Get pending withdrawals (admin) - DEBUGGED VERSION
app.get('/api/admin/pending-withdrawals', adminAuth, async (req, res) => {
  try {
    ...
    // DEBUGGED: Handle null users safely
    const safeWithdrawals = withdrawals.map(wdl => ({
      ...wdl,
      user_details: wdl.user ? {
        name: wdl.user.full_name || 'Unknown User',
    ...
    res.json(formatResponse(true, 'Pending withdrawals retrieved', { withdrawals: safeWithdrawals }));
  } catch (error) {
...
// Get pending KYC (admin) - DEBUGGED VERSION
app.get('/api/admin/pending-kyc', adminAuth, async (req, res) => {
  try {
    ...
    // DEBUGGED: Handle null users safely
    const safeKYCSubmissions = kycSubmissions.map(kyc => ({
      ...kyc,
      user_details: kyc.user ? {
        name: kyc.user.full_name || 'Unknown User',
    ...
    res.json(formatResponse(true, 'Pending KYC retrieved', { kyc: safeKYCSubmissions }));
  } catch (error) {
