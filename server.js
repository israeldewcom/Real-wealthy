...
// DAILY INTEREST STARTS IMMEDIATELY AFTER ADMIN APPROVAL & UPDATES
EVERY 24 HOURS
...
// UPDATED DURATIONS: First three 20 days, next three 15 days, rest 9
days
...
// ENHANCED: ALL INVESTMENTS NOW AUTO-SEND TO ADMIN FOR APPROVAL
// ENHANCED: USER BALANCE NOT DEDUCTED UNTIL ADMIN APPROVES INVESTMENT
...
const config = {
  ...
  // ENHANCED: Investment Approval System
  autoSendInvestmentsToAdmin: true, // All investments go to admin for approval
  deductBalanceOnApproval: true, // Only deduct balance when admin approves
...
console.log(`- Auto Send Investments to Admin: ${config.autoSendInvestmentsToAdmin}`);
console.log(`- Deduct Balance on Approval: ${config.deductBalanceOnApproval}`);
...
const investmentSchema = new mongoose.Schema({
  ...
  interest_added_count: { type: Number, default: 0 }, // How many times interest has been added
  total_interest_days: { type: Number, default: 0 }, // Total days interest should be added
  ...
  // ENHANCED: Track if balance was deducted
  balance_deducted: { type: Boolean, default: false },
  pending_balance_deduction: { type: Boolean, default: false },
      ...
      if (status === 'completed') {
        switch (type) {
          ...
          case 'investment':
            ...
            // ENHANCED: Only deduct if balance_deducted is false
            if (!metadata.balance_deducted) {
              ...
              console.log(`📈 Investment amount ${investmentAmount} already deducted from balance`);
            }
            break;
...
// ==================== ENHANCED INVESTMENT CREATION - AUTO SEND TO ADMIN ====================
app.post('/api/investments', auth, upload.single('payment_proof'), [
  ...
  try {
    ...
    // ENHANCED: Check if user has enough balance for the investment
    ...
    const nextInterestDate = new Date(Date.now() + 24 * 60 * 60 * 1000); // First interest after 24 hours
    
    // ENHANCED: All investments go to admin for approval
    const investment = new Investment({
      user: userId,
      plan: plan_id,
      amount: investmentAmount,
      status: 'pending', // ENHANCED: Always set to pending
      start_date: new Date(),
      end_date: endDate,
      ...
      payment_verified: false, // ENHANCED: Set to false, will be true when admin approves
      balance_deducted: false, // ENHANCED: Track if balance was deducted
      pending_balance_deduction: true // ENHANCED: Pending balance deduction
    });
    
    await investment.save();
    
    // ENHANCED: DO NOT deduct investment amount from user's balance yet
    // The balance will be deducted when admin approves the investment
    
    // ENHANCED: Create a pending transaction for tracking
    await createTransaction(
      userId,
      'investment',
      -investmentAmount,
      `Investment in ${plan.name} plan - Pending Admin Approval`,
      'pending',
      {
        investment_id: investment._id,
        plan_name: plan.name,
        ...
        balance_deducted: false, // ENHANCED: Not deducted yet
    ...
    await createNotification(
      userId,
      'Investment Created',
      `Your investment of ₦${investmentAmount.toLocaleString()} in ${plan.name} has been created and is pending admin approval. Your balance will be deducted once approved.`,
      'investment',
      '/investments'
    );
    
    // ENHANCED: Always notify admins about new pending investment
    emitToInvestmentAdmins('new-pending-investment', {
      ...
      amount: investmentAmount,
      plan_name: plan.name,
      payment_proof: proofUrl ? true : false,
      timestamp: new Date().toISOString()
    });
    
    // Also notify regular admin room
    emitToAdmins('new-investment-request', {
      investment_id: investment._id,
      user_id: userId,
      amount: investmentAmount,
      plan_name: plan.name,
      requires_approval: true,
      has_proof: proofUrl ? true : false
    });
    
    res.status(201).json(formatResponse(true, 'Investment created successfully! Awaiting admin approval.', {
      investment: {
        ...investment.toObject(),
        plan_name: plan.name,
      ...
      message: 'Your investment is pending admin approval. Your balance will be deducted once approved.'
    }));
...
// ==================== ENHANCED INVESTMENT APPROVAL WITH BALANCE DEDUCTION ====================
app.post('/api/admin/investments/:id/approve', adminAuth, [
  ...
  try {
    ...
    // ENHANCED: Check if user still has enough balance
    const freshUser = await User.findById(investment.user._id);
    if (!freshUser) {
      return res.status(404).json(formatResponse(false, 'User not found'));
    }
    
    if (investment.amount > freshUser.balance) {
      return res.status(400).json(formatResponse(false,
        `User does not have enough balance for this investment. Required: ₦${investment.amount.toLocaleString()}, Available: ₦${freshUser.balance.toLocaleString()}`));
    }
    ...
    investment.balance_deducted = true; // ENHANCED: Mark as deducted
    investment.pending_balance_deduction = false; // ENHANCED: No longer pending
    
    await investment.save();
    
    // ENHANCED: Deduct investment amount from user's balance
    await createTransaction(
      investment.user._id,
      'investment',
      -investment.amount,
      ...
      'completed',
      {
        ...
        next_interest_date: nextInterestDate,
        balance_deducted: true // ENHANCED: Mark as deducted
      }
    );
    ...
    await Transaction.findOneAndUpdate(
      ...
      {
        status: 'completed',
        description: `Investment in ${investment.plan.name} plan - Approved by Admin`,
        metadata: {
          ...investment.metadata,
          approved_by: adminId,
    ...
    await createNotification(
      investment.user._id,
      'Investment Approved',
      `Your investment of ₦${investment.amount.toLocaleString()} in ${investment.plan.name} has been approved and is now active. First interest of ₦${addInterestResult.dailyEarning?.toLocaleString() || '0'} has been credited. Your balance has been deducted: ₦${investment.amount.toLocaleString()}`,
      'investment',
      '/investments'
    );
    ...
    res.json(formatResponse(true, 'Investment approved successfully', {
      ...
      next_interest_date: investment.next_interest_date,
      balance_deducted: true,
      user_balance_after: freshUser.balance - investment.amount
    }));
...
app.post('/api/admin/investments/:id/reject', adminAuth, [
  ...
  try {
    ...
    // ENHANCED: Update the investment status without refunding (since balance was never deducted)
    investment.status = 'rejected';
    investment.rejected_at = new Date();
    investment.rejected_by = adminId;
    investment.rejection_reason = rejection_reason;
    investment.balance_deducted = false; // ENHANCED: Balance was never deducted
    investment.pending_balance_deduction = false; // ENHANCED: No longer pending
    
    await investment.save();
    
    // ENHANCED: Update the pending transaction to cancelled
    await Transaction.findOneAndUpdate(
      { 
        related_investment: investmentId,
        type: 'investment',
    ...
    await AdminAudit.create({
      ...
      details: {
        ...
        user_email: investment.user.email,
        rejection_reason,
        refund_processed: false, // ENHANCED: No refund needed
        balance_deducted: false
      },
      ip_address: req.ip,
      user_agent: req.headers['user-agent']
    ...
    res.json(formatResponse(true, 'Investment rejected successfully', {
      investment: investment.toObject(),
      refund_amount: 0, // ENHANCED: No refund since balance wasn't deducted
      user_balance_updated: false,
      message: 'Investment rejected. User balance was not affected since it was never deducted.'
    }));
...
app.post('/api/admin/withdrawals/:id/approve', adminAuth, [
  ...
  try {
    ...
    await Transaction.findOneAndUpdate(
      ...
      {
        status: 'completed',
        description: `Withdrawal via ${withdrawal.payment_method} - Approved by Admin`,
        metadata: {
          ...withdrawal.metadata,
          approved_by: adminId,
          approved_at: new Date(),
  ...
  try {
    const systemStatus = {
      ...
      config: {
        ...
        autoSendInvestmentsToAdmin: config.autoSendInvestmentsToAdmin,
        deductBalanceOnApproval: config.deductBalanceOnApproval,
        planDurations: {
          firstThree: '20 days',
          nextThree: '15 days',
          remaining: '9 days'
...
const startServer = async () => {
  try {
    ...
    server.listen(config.port, () => {
      ...
      console.log('1. ✅ ALL INVESTMENTS AUTO-SEND TO ADMIN FOR APPROVAL');
      console.log('2. ✅ USER BALANCE NOT DEDUCTED UNTIL ADMIN APPROVES');
      console.log('3. ✅ DAILY INTEREST STARTS IMMEDIATELY AFTER ADMIN APPROVAL');
      console.log('4. ✅ INTEREST UPDATED EVERY 24 HOURS UNTIL EXPIRY');
      console.log('5. ✅ INTEREST RATES UPDATED: 3500 PLAN = 15%, ALL OTHERS +5%');
      console.log('6. ✅ UPDATED PLAN DURATIONS: First three 20 days, next three 15 days, rest 9 days');
      console.log('7. ✅ MINIMUM WITHDRAWAL UPDATED: ₦4,000');
      console.log('8. ✅ ADMIN CAN REJECT DEPOSITS AND INVESTMENTS');
      console.log('9. ✅ ADMIN CAN SUSPEND/ACTIVATE/REJECT USER ACCOUNTS');
      console.log('10.✅ ADVANCED USER ACCOUNT STATUS MANAGEMENT');
      console.log('11.✅ ALL WITHDRAWALS REQUIRE ADMIN APPROVAL');
      console.log('12.✅ REFERRAL COMMISSIONS: Only on first investment');
      console.log('13.✅ REAL-TIME ADMIN NOTIFICATIONS FOR ALL ACTIONS');
      console.log('============================================\n');
      ...
      console.log('1. ✅ ALL INVESTMENTS AUTO-SENT TO ADMIN (WITH OR WITHOUT PROOF)');
      console.log('2. ✅ BALANCE DEDUCTION ONLY ON ADMIN APPROVAL');
      console.log('3. ✅ USER ACCOUNT SUSPENSION WITH REASON');
      console.log('4. ✅ USER ACCOUNT REJECTION SYSTEM');
      console.log('5. ✅ ACCOUNT ACTIVATION/RESTORATION');
      console.log('6. ✅ BALANCE MANAGEMENT (ADD/SUBTRACT/SET)');
      console.log('7. ✅ INVESTMENT APPROVAL/REJECTION WITH BALANCE DEDUCTION');
      console.log('8. ✅ DEPOSIT APPROVAL/REJECTION');
      console.log('9. ✅ WITHDRAWAL APPROVAL/REJECTION');
      console.log('10.✅ COMPREHENSIVE FINANCIAL REPORTS');
      console.log('11.✅ REAL-TIME USER FINANCIAL SUMMARY');
      ...
      console.log('✅ INVESTMENT SYSTEM ENHANCED: AUTO-SEND TO ADMIN');
      console.log('✅ BALANCE DEDUCTION SYSTEM: ONLY ON ADMIN APPROVAL');
